...
function buildReviewData({ title, generatedAt, sourcePath, repoPath }) {
  const reviewMode = 'working-tree-vs-head-plus-untracked';
...
function main() {
  ...
  if (!templatePath || !outputPath) {
    throw new Error('Usage: build-review-html.js --template <path> --out <path> [--title <title>] [--generated-at <timestamp>] [--source-path <path>] [--repo <path>]');
  }

  const template = fs.readFileSync(templatePath, 'utf8');
  const data = buildReviewData({ title, generatedAt, sourcePath, repoPath });
