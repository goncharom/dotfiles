---
name: code-review
description: Generates a minimal commentable HTML code review for the current git repo by rendering all current changes into ./.miru/. Use when the user wants to review code changes, inspect a diff in HTML, or leave line comments before applying feedback.
...
- The HTML should render all current repo changes, including untracked files.
...
2. Decide a clear review title and slug.
3. Generate a timestamp for the filename.
4. Create `./.miru/` if needed.
5. Build the HTML from `templates/review-template.html` using `tools/build-review-html.js`.
6. Write the new HTML file to `./.miru/`.
7. Reply with:
...
- The builder auto-detects the repo root and review mode metadata from the repo state.
- Tracked changes are collected from `git diff HEAD`.
- Untracked files are added as synthetic all-add sections.
